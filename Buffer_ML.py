...
def dataPrepare(filePath,flag=0):
    ...
    #data.dropna(axis=0, how='any', inplace=True)  axis=0按行删除，=1按列删除
    #data.dropna(axis=1, how='any', inplace=True) 用于删除缺省值的，行或者列有空值将会被删掉
...
# save results
def saveResults(GeoID, y_pred, result_file):
    results = np.vstack((GeoID,y_pred))
    results = np.transpose(results)
    header_string = 'GeoID, y_pred'
    np.savetxt(result_file, results, header = header_string, fmt = '%d,%0.5f',delimiter = ',')
    print('Saving file Done!')

sampleData = r'Buffer_train.csv'
...
model3 = GradientBoostingClassifier(n_estimators=n_estimator)
...
# plt.axis('square')#正方形的坐标框
# plt.xlim([0, 1.05])
# plt.ylim([0, 1.05])
# plt.plot(fpr1, tpr1, linestyle='-', label='Buffer+SVM(AUC=%0.3f)' %test_auc1, color='b')
# plt.plot(fpr2, tpr2, linestyle='-', label='Buffer+RF(AUC=%0.3f)' %test_auc2, color='g')
# plt.plot(fpr3, tpr3, linestyle='-', label='Buffer+GBDT(AUC=%0.3f)' %test_auc3, color='r')
# plt.xlabel('False positive rate',fontsize=12)
# plt.ylabel('True positive rate',fontsize=12)
# plt.text(0.8, 0.4,'(b)',fontsize=14)
...
Buffer_SVM_result = './Buffer_SVM_result.txt'
#saveResults(GeoID, y_pred1, y_pred_proba1, Buffer_SVM_result)
saveResults(GeoID, y_pred_proba1, Buffer_SVM_result)

y_pred_proba2 = model2.predict_proba(X)[:,1]
Buffer_RF_result = './Buffer_RF_result.txt'
saveResults(GeoID, y_pred_proba2, Buffer_RF_result)

y_pred_proba3 = model3.predict_proba(X)[:,1]
Buffer_GBDT_result = './Buffer_GBDT_result.txt'
saveResults(GeoID, y_pred_proba3, Buffer_GBDT_result)