...
plt.xlim([0, 1.05])
plt.ylim([0, 1.05])
plt.plot(fpr1, tpr1, linestyle='-', label='RDM+SVM(AUC=%0.3f)' %test_auc1, color='b')
plt.plot(fpr2, tpr2, linestyle='-', label='RDM+RF(AUC=%0.3f)' %test_auc2, color='g')
plt.plot(fpr3, tpr3, linestyle='-', label='RDM+GBDT(AUC=%0.3f)' %test_auc3, color='r')
...
RDM_SVM_result = './RDM_SVM_result.txt'
#saveResults(GeoID, y_pred1, y_pred_proba1, result_file1)
saveResults(GeoID, y_pred_proba1, RDM_SVM_result)

y_pred_proba2 = model2.predict_proba(X)[:,1]
RDM_RF_result = './RDM_RF_result.txt'
saveResults(GeoID, y_pred_proba2, RDM_RF_result)

y_pred_proba3 = model3.predict_proba(X)[:,1]
RDM_GBDT_result = './RDM_GBDT_result.txt'
saveResults(GeoID, y_pred_proba3, RDM_GBDT_result)